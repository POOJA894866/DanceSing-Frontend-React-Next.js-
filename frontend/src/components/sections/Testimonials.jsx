...
const Testimonials = ({ data }) => {
  ...
  const Star = () => (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="#F4D400">
  ...
  return (
    <section className="section testimonials" id={data.id || 'testimonials'} style={{ background: '#283466', padding: '100px 0', overflow: 'hidden' }}>
      <div style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', padding: '0 24px', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: '44px', fontFamily: 'Sora', fontWeight: '300', color: 'white', marginBottom: '24px', lineHeight: '1.2', letterSpacing: '-0.02em' }}>
          Our Experience Is Your Advantage
        </h2>

        {/* Rating Summary Badge */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '12px', background: '#F6F8F999', border: '1px solid rgba(255,255,255,0.12)', padding: '20px 32px', borderRadius: '12px', marginBottom: '64px' }}>
          <div style={{ display: 'flex', gap: '2px' }}>
            {[...Array(5)].map((_, i) => <Star key={i} />)}
          </div>
          <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '2px' }}>
            <span style={{ color: 'black', fontSize: '15px', fontWeight: '600' }}>4.9 out of 5 Stars</span>
            <span style={{ color: 'black', fontSize: '13px', fontWeight: '400' }}>from 47 verified reviews</span>
          </span>
        </div>
            ...
            <div key={i} style={{ background: 'white', padding: '40px 32px', borderRadius: '20px', textAlign: 'left', minWidth: '340px', maxWidth: '340px', flexShrink: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', boxShadow: '0 15px 35px rgba(0,0,0,0.1)' }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  ...
                  <span style={{ color: item.tagColor, background: `${item.tagColor}15`, border: `1px solid ${item.tagColor}40`, padding: '4px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: '800', letterSpacing: '0.05em' }}>
                    {item.tag}
                  </span>
                </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginTop: 'auto' }}>
                <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: item.tagColor, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: '700' }}>
                  {item.initials}
                </div>
                <div>
